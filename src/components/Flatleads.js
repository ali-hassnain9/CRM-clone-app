...
import { getFieldParams } from '../screens/FilterScreen'
  ...
  const fetch = async () => {
    try {
      if (Object.keys(filtersObj.state).length) {
        ...
        let leadsResponse = await axios.get(`https://dev2.empgautos.com/api/crm/crm_leads?page=${Allleads.currentPage+1}`+values,
          { headers: header })
        let arr = Allleads.data
        let newArray = arr.concat(leadsResponse.data.crm_leads)
        dispatch(fetchLeads({
          data: newArray, maxPage: leadsResponse.data.pagination.total_pages,
          currentPage: leadsResponse.data.pagination.current_page,
          nextPage: leadsResponse.data.pagination.next_page
        }))
      }
      else {
        const leadsResponse = await axios.get(`https://dev2.empgautos.com/api/crm/crm_leads?page=${Allleads.currentPage+1}`,
          { headers: header })
        let arr = Allleads.data
        let newArray = arr.concat(leadsResponse.data.crm_leads)
        dispatch(fetchLeads({
