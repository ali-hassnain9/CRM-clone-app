  ...
  const filterOptions = (item) => {
    ...
    Object.keys(item).map((parentItem) => {
        ...
        if (parentItem === 'lead_ref_id') {
          selectedValues = {
            id: Math.random(), name: item[parentItem], slugs: item[parentItem], type: parentItem
          }
          ...
          item[parentItem].map((elem) => {
            selectedValues = {
              id: elem.id, type: parentItem, name: elem.name, slugs: elem.slugs
            }
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          return (
            <TouchableOpacity
              onPress={() => {
                if (item.type === 'lead_ref_id' || item.type === 'lead_client_email') {
            ...
            >
              <View style={styles.chip}>
                ...
                <Image source={IC_CLOSE} resizeMode={'contain'} style={styles.filterIcon}/>
              </View>
            </TouchableOpacity>
          )
        }}
